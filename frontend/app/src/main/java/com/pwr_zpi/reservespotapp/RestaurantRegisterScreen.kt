...
@Composable
fun RestaurantRegisterScreen(navController: NavHostController) {
...
fun sendRegisterRequest(
    ...
    coroutineScope.launch {
        try {
            val response = RetrofitClient.authApi.restaurantRegister(
                RestaurantRegisterRequest(name = name, email = email, password = password)
            )
